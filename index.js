...
async function processItem(item){
  ...
  if(item.kind==='ATTENDANCE_CHALLENGE'){
    ...
    const embed=new EmbedBuilder().setColor(0xD3A93A).setTitle('⏱️ Meta de serviço concluída').setDescription(p.dm_message||'Escolha se deseja sair de serviço ou confirmar e continuar em hora extra. Enquanto não escolher, o tempo adicional fica em horas divergentes.').addFields({name:'✅ Horas normais',value:'A meta cumprida permanece registrada.',inline:true},{name:'⚠️ Horas divergentes',value:'Tempo adicional sem confirmação.',inline:true}).setFooter({text:'Polícia Militar do Estado do Rio de Janeiro - Reduto Online'}).setTimestamp();
