...
async function upsertPublicPanel(forceChannel=null){
 ...
 const embeds=[];if(d.public_top_image_url)embeds.push(new EmbedBuilder().setColor(0x1595D3).setImage(d.public_top_image_url));embeds.push(new EmbedBuilder().setColor(0x1595D3).setTitle('🏛️ Portal PMERJ • Roleplay').setDescription('Portal comunitário do projeto de roleplay PMERJ.\n\n📋 **Centro de Gestão**\n📝 **Registro de membros**\n🛡️ **Canal de denúncias**\n🎓 **Formação e recrutamento**\n🚔 **Operações e atividade**\n\n*Projeto não oficial, sem vínculo institucional ou governamental com a PMERJ real.*').setFooter({text:'Portal PMERJ • Centro de Gestão'}));if(d.public_bottom_image_url)embeds.push(new EmbedBuilder().setColor(0x1595D3).setImage(d.public_bottom_image_url));
 const row=new ActionRowBuilder().addComponents(new ButtonBuilder().setStyle(ButtonStyle.Link).setURL(SITE_URL).setLabel('Acessar o Portal'),new ButtonBuilder().setStyle(ButtonStyle.Link).setURL(`${SITE_URL.replace(/\/$/,'')}/register`).setLabel('Registro'));
